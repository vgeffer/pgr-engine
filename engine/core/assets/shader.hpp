...
namespace pgreng::assets {
    class shader_stage : public asset {
        ...
        public:
            ...
            shader_stage(const std::string_view& shader_source, uint32_t type);
            ...
            inline uint32_t type_bitmask() const { return m_type_bitmask; }
            ...
            explicit operator uint32_t() { return m_program; }

        private:
            template <typename T>
            void m_set_uniform_value(int location, const T& val);

        private:
            uint32_t m_type_bitmask;  ///< Shader type bitmask
            uint32_t m_program;           ///< OpenGL shader program object
    };
}
