...
#include "core/runtime/memory.hpp"
#include <iostream>
#include <memory>
#include <stdexcept>
#include <string>
...
namespace pgreng::core::assets {
    ...
    class loader {
        ...
        public:
            loader() { spInstance = this; }
            ~loader() { spInstance = nullptr; }
            ...
            template<class T>
                ...
                if (policy == caching_policy::NO_CACHE) {
                    common::debug::log << "Loading " << path << " - cache bypassed" << std::endl;
                    return std::make_shared<T>(path);
                }

                runtime::Memory::allocRefcount<T>();

                auto object = s_instance->m_cache[path].lock();
                if (!object) {

                    common::debug::log << "Loading " << path << " - cache miss, loading from file" << std::endl;
        ...
        private:
            inline static Loader* spInstance = nullptr;
