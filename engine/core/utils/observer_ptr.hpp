...
namespace utils {
    ...
    namespace _internal {
        ...
        struct observable_state {
            uint32_t active_observers : 31;    ///< Number of active observers of this observable pointer
            uint32_t ptr_valid : 1;             ///< Flag, if upstream observable pointer is still valid
        };
    }
    ...
    class observable_ptr {
        ...
        public:
            ...
            constexpr T& operator*() const { 
                DEBUGIF (m_data == nullptr) throw std::runtime_error("Trying to dereference an invalid pointer");
                return *m_data; 
            }               
            constexpr T* operator->() const { 
                DEBUGIF (m_data == nullptr) throw std::runtime_error("Trying to dereference an invalid pointer");
            ...
            constexpr observer_ptr<T> observer() {
                DEBUGIF (m_state == nullptr || !m_state->ptr_valid)
