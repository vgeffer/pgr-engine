...
namespace assets {
    ...
    class texture : public asset {
        public: 
            ...
            inline uint64_t texture_handle() const { return m_texture_handle; }
        ...
        private:
            ...
            uint32_t m_texture_obj;                           ///< OpenGL texture object
            uint64_t m_texture_handle;                      ///< Texture's handle in OpenGL memory
            utils::gpu_allocator::handle m_buffer_handle;   ///< Handle to the texture storage in an internal buffer
            int m_texture_index;                          ///< Internal index by which the texture could be accessed in shader
